...
" iced-vim
" --- Vim Iced: enable only in Clojure projects ---
function! SetupVimIced()
  " Enable Iced’s default keymaps
  let g:iced_enable_default_key_mappings = v:true

  if executable('lein')
	  call iced#nrepl#connect#auto()
  endif

  " Optional: tweak Iced behavior here
  " let g:iced#nrepl#auto_start = v:true
  " let g:iced#eval#inline#enable = v:true
endfunction

augroup vim_iced_setup
  autocmd!
  " Trigger only for Clojure files
  autocmd FileType clojure call SetupVimIced()

  " Optional: activate only if inside a Clojure project (has deps.edn, project.clj, or shadow-cljs.edn)
  autocmd BufReadPost,BufNewFile *
        \ if filereadable('deps.edn') || filereadable('project.clj') || filereadable('shadow-cljs.edn') |
        \   call SetupVimIced() |
        \ endif
augroup END
